namespace UniLuaInterface 
{
	...
	public class ObjectTranslator 
	{
		...
		private CSharpFunctionDelegate registerTableFunction,unregisterTableFunction,getMethodSigFunction,
			getConstructorSigFunction,importTypeFunction,loadAssemblyFunction, test;
		 ...
		 */
		private void createLuaObjectList(ILuaState luaState) 
		{
		 ...
		 * passed to Lua
		 */
		private void createIndexingMetaFunction(ILuaState luaState) 
		{
            ...
            LuaDLL.lua_rawset(luaState, (int) LuaDef.LUA_REGISTRYINDEX);
		}
		/*
		 ...
		 */
		private void setGlobalFunctions(ILuaState luaState)
		{
		 ...
		 */
		private void createFunctionMetatable(ILuaState luaState) 
		{
		 ...
		 */
        internal void throwError(ILuaState luaState, object e)
        {
		 ...
		 */
		private int loadAssembly(ILuaState luaState) 
		{            
            try
            {
                ...
                try
                {
                    //!MODIFY BY hyf042, LoadWithPartialName has been rejected, use Load instead
		 ...
		 * type is not found.
		 */
		private int importType(ILuaState luaState) 
		{
		 ...
		 */
		private int registerTable(ILuaState luaState) 
		{
			if(LuaDLL.lua_type(luaState,1)==LuaType.LUA_TTABLE) 
			{
		 ...
		 */
		private int unregisterTable(ILuaState luaState) 
		{
			try 
			{
		 ...
		 */
		private int getMethodSignature(ILuaState luaState) 
		{
			...
			try 
			{
				...
				pushFunction(luaState,new CSharpFunctionDelegate((new LuaMethodWrapper(this,target,klass,method)).call));
			} 
			catch(Exception e) 
			{
				throwError(luaState,e);
		 ...
		 */
		private int getConstructorSignature(ILuaState luaState) 
		{
			IReflect klass=null;
			int udata=LuaDLL.luanet_checkudata(luaState,1,"luaNet_class");
			if(udata!=-1) 
			...
			try 
			{
				...
				pushFunction(luaState,new CSharpFunctionDelegate((new LuaMethodWrapper(this,null,klass,constructor)).call));
			} 
			catch(Exception e) 
			{
				throwError(luaState,e);
		 ...
		 */
		internal void pushType(ILuaState luaState, Type t) 
		{
		 ...
		 */
		internal void pushObject(ILuaState luaState, object o, string metatable) 
		{
			int index = -1;
			// Pushes nil
			if(o==null) 
		 ...
		 * returns null.
		 */
		internal object getAsType(ILuaState luaState,int stackPos,Type paramType) 
		{
		 ...
		 */
		internal object getObject(ILuaState luaState,int index) 
		{
			LuaType type=LuaDLL.lua_type(luaState,index);
			switch(type) 
			{
		 ...
		 */
		internal LuaUserData getUserData(ILuaState luaState,int index) 
		{
			LuaDLL.lua_pushvalue(luaState,index);
		 ...
		 */
		internal LuaFunction getFunction(ILuaState luaState,int index) 
		{
			LuaDLL.lua_pushvalue(luaState,index);
		 ...
		 */
		internal object getNetObject(ILuaState luaState,int index) 
		{
		 ...
		 * delegates as is.
		 */
		internal object getRawNetObject(ILuaState luaState,int index) 
		{
		 ...
		 * of elements pushed.
		 */
		internal int returnValues(ILuaState luaState, object[] returnValues) 
		{
		 ...
		 */
		internal object[] popValues(ILuaState luaState,int oldTop) 
		{
		 ...
		 */
		internal object[] popValues(ILuaState luaState,int oldTop,Type[] popTypes) 
		{
			int newTop=LuaDLL.lua_gettop(luaState);
			if(oldTop==newTop) 
			{
		 ...
		 */
		internal void push(ILuaState luaState, object o) 
		{
			if(o==null) 
			{
				LuaDLL.lua_pushnil(luaState);
			...
			{
				pushFunction(luaState,(CSharpFunctionDelegate)o);
			} 
